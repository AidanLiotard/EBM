...
from ptt.ising_ising.classes import IIRBMSampler
from ptt.bernoulli_bernoulli.classes import BBRBMSampler
...
@pytest.fixture
def sample_params_class_iirbm():
    weight_matrix = torch.randn(pytest.NUM_VISIBLES, pytest.NUM_HIDDENS)
    vbias = torch.randn(pytest.NUM_VISIBLES)
    hbias = torch.randn(pytest.NUM_HIDDENS)
    params = IIRBM(weight_matrix, vbias, hbias)
    return params


@pytest.fixture
def sample_sampler_iirbm():
    weight_matrix = torch.randn(pytest.NUM_VISIBLES, pytest.NUM_HIDDENS)
    vbias = torch.randn(pytest.NUM_VISIBLES)
    hbias = torch.randn(pytest.NUM_HIDDENS)
    params_1 = IIRBM(weight_matrix, vbias, hbias)
    weight_matrix = torch.randn(pytest.NUM_VISIBLES, pytest.NUM_HIDDENS)
    vbias = torch.randn(pytest.NUM_VISIBLES)
    hbias = torch.randn(pytest.NUM_HIDDENS)
    params_2 = IIRBM(weight_matrix, vbias, hbias)
    sampler= IIRBMSampler(
        list_model =[params_1, params_2],
        pre_sampler = None,
        increment=pytest.INCREMENT,
        num_steps=pytest.GIBBS_STEPS,
        log_z_init=0,
        device='cpu', 
        dtype=torch.float32)
    sampler.init_random_chains(pytest.NUM_CHAINS)
    return sampler

@pytest.fixture
def sample_sampler_bbrbm():
    weight_matrix = torch.randn(pytest.NUM_VISIBLES, pytest.NUM_HIDDENS)
    vbias = torch.randn(pytest.NUM_VISIBLES)
    hbias = torch.randn(pytest.NUM_HIDDENS)
    params_1 = BBRBM(weight_matrix, vbias, hbias)
    weight_matrix = torch.randn(pytest.NUM_VISIBLES, pytest.NUM_HIDDENS)
    vbias = torch.randn(pytest.NUM_VISIBLES)
    hbias = torch.randn(pytest.NUM_HIDDENS)
    params_2 = BBRBM(weight_matrix, vbias, hbias)
    sampler= BBRBMSampler(
        list_model =[params_1, params_2],
        pre_sampler = None,
        increment=pytest.INCREMENT,
        num_steps=pytest.GIBBS_STEPS,
...
@pytest.fixture
def sample_chains_iirbm():
    mean_visible = torch.ones(pytest.NUM_CHAINS, pytest.NUM_VISIBLES) / 2
    mean_hidden = torch.ones(pytest.NUM_CHAINS, pytest.NUM_HIDDENS) / 2
    visible = 2*torch.bernoulli(mean_visible)-1
    hidden = 2*torch.bernoulli(mean_hidden)-1
    chains = dict(
        visible=visible,
        hidden=hidden,
